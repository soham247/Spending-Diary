...
const processMonthlyData = (expenses: SimpleExpense[]) => {
  ...
  expenses.forEach((expense: SimpleExpense) => {
    ...
    if (typeof monthlyData[monthYear][expense.tag] === "number") {
      monthlyData[monthYear][expense.tag] =
        (monthlyData[monthYear][expense.tag] as number) + expense.amount;
    } else {
...
async function POST(request: NextRequest) {
  try {
    ...
    console.error("Error retrieving premium analytics:");
    return NextResponse.json(
...
function processTrendsData(expenses: SimpleExpense[], timeframe: string) {
  ...
  expenses.forEach((expense: SimpleExpense) => {
    ...
    periodsData[periodKey].maximum = Math.max(
      periodsData[periodKey].maximum,
      expense.amount
    );
  });
...
function processForecastData(expenses: SimpleExpense[], timeframe: string) {
  ...
  expenses.forEach((expense) => {
    ...
    periodsData[periodKey].actual =
      (periodsData[periodKey].actual || 0) + expense.amount;
  });
...
function calculateAverageGrowth(
  periods: Array<{ period: string; actual: number | null }>
) {
